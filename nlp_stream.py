...
def main():
    ...
    <h3 style="color:blue">텍스트 분석 및 감정분석</h1>
    </div>
    """
    title="자연어처리프로젝트"
    st.title(title)
    ...
    #셀렉트박스의 다양한 결과물의 menu 변수에 저장하여
    #선택에 따라 sidebar에 text 출력
    menu=st.sidebar.selectbox("메뉴",["텍스트 분석","감성 분석","번역","이 프로젝트에 대하여"])

    if menu =="텍스트 분석":
        raw_text=st.text_area("text_input","여기에 영어 텍스트를 입력하세요",height=300)
        ...
        if st.button("start"):
            if len(raw_text) ==0:
                ...
                with col1:
                    ...
                    with st.expander("basic info nlp"):
                        #접었다 폈다
                        word_desc=nt.TextFrame(raw_text).word_stats()
                        result_desc={"Len of Text:",word_desc["Length of Text"],
                                    "Num df Vowels:",word_desc["Num of Vowels"],
                                    "Num of Stopwords:",word_desc["Num of Stopwords"]}
                        st.write(result_desc)

                
                with col2:
                    with st.expander("Pre_processed-text"):
